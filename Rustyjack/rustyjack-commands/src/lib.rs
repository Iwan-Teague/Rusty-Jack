...
pub enum HotspotCommand {
    ...
    /// Show hotspot status
    Status,
    /// Disconnect a hotspot client by MAC
    DisconnectClient(HotspotDisconnectArgs),
    /// Update hotspot MAC blacklist
...
pub enum WifiCommand {
    ...
    /// Randomize the interface MAC address
    MacRandomize(WifiMacRandomizeArgs),
    /// Set the interface MAC using a vendor OUI
    MacSetVendor(WifiMacSetVendorArgs),
    /// Set the interface MAC to a specific value
    MacSet(WifiMacSetArgs),
    /// Restore the interface MAC address
    MacRestore(WifiMacRestoreArgs),
    /// Set the interface TX power (dBm)
    TxPower(WifiTxPowerArgs),
...
pub struct WifiTxPowerArgs {
    #[arg(long)]
    pub interface: String,
    /// Desired TX power in dBm (e.g., 5, 12, 18)
    #[arg(long)]
    pub dbm: u32,
}
...
pub enum WifiProfileCommand {
    ...
    List,
    /// Show stored credentials for a Wi-Fi profile
    Show(WifiProfileShowArgs),
...
pub struct UsbMountArgs {
    ...
    pub device: String,
    /// Mount mode
    #[arg(long, value_enum, default_value_t = UsbMountMode::ReadOnly)]
    pub mode: UsbMountMode,
    /// Optional preferred mount directory name
    #[arg(long)]

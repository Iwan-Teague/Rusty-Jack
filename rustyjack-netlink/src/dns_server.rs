...
impl DnsServer {
    ...
    pub fn start(&mut self) -> Result<()> {
        Err(DnsError::InvalidConfig(
            "DNS server only supported on Linux".to_string(),
        ))
    }
    ...
    fn send_response(
        ...
        socket.send_to(&response, client).map_err(|e| DnsError::SendFailed {
            client,
            source: e,
        })?;
...
mod tests {
    ...
    #[test]
    fn test_parse_name_single_label() {

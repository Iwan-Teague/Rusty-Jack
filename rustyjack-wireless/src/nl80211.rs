...
pub fn set_interface_state(name: &str, up: bool) -> Result<()> {
    ...
    if !status.success() {
        return Err(WirelessError::Interface(
            format!("Failed to set interface {} {}", name, state)
        ));
    }
...
pub fn set_interface_type_iw(name: &str, mode: Nl80211IfType) -> Result<()> {
    ...
    let mode_str = match mode {
        ...
        _ => return Err(WirelessError::Unsupported(
            format!("Interface type {:?} not supported via iw", mode)
        )),
    };
    ...
    if !status.success() {
        return Err(WirelessError::MonitorMode(
            format!("Failed to set {} to {} mode", name, mode_str)
        ));
    }
...
pub fn set_channel_iw(name: &str, channel: u8) -> Result<()> {
    ...
    if !status.success() {
        ...
        if !status2.success() {
            return Err(WirelessError::Channel(
                format!("Failed to set channel {} on {}", channel, name)
            ));
        }
    }
...
pub fn set_frequency_iw(name: &str, freq_mhz: u32) -> Result<()> {
    ...
    if !status.success() {
        return Err(WirelessError::Channel(
            format!("Failed to set frequency {} MHz on {}", freq_mhz, name)
        ));
    }
...
pub fn kill_interfering_processes() -> Result<()> {
    ...
    let processes = ["wpa_supplicant", "NetworkManager", "dhclient", "avahi-daemon"];
...
pub fn get_mac_address(name: &str) -> Result<[u8; 6]> {
    ...
    if parts.len() != 6 {
        return Err(WirelessError::Interface(format!("Invalid MAC format: {}", mac_str)));
    }
...
pub fn set_mac_address(name: &str, mac: &[u8; 6]) -> Result<()> {
    ...
    if !status.success() {
        return Err(WirelessError::Interface(
            format!("Failed to set MAC address to {}", mac_str)
        ));
    }

...
use crate::txpower::{TxPowerManager, TxPowerLevel};
...
impl PassiveManager {
    ...
    pub fn enable(&mut self, interface: &str) -> Result<String> {
            ...
            if let Ok(output) = airmon {
                if !output.status.success() {
                    ...
                    return Err(EvasionError::InterfaceError(
                        format!("Failed to create monitor interface: {}", stderr)
                    ));
                }
            } else {
                let stderr = String::from_utf8_lossy(&create.stderr);
        ...
        log::info!("Enabled passive monitor mode on {} -> {}", interface, mon_name);
        Ok(mon_name)
    }
    ...
    pub fn set_channel(&self, interface: &str, channel: u8) -> Result<()> {
        ...
        if !output.status.success() {
            ...
            return Err(EvasionError::InterfaceError(
                format!("Failed to set channel {}: {}", channel, stderr)
            ));
        }

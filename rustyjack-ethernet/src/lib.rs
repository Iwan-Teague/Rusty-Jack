...
/// Perform a TCP connect scan with source binding and cancellation support.
pub fn quick_port_scan_with_source_cancellable(
    target: Ipv4Addr,
    ports: &[u16],
    timeout: Duration,
    source_ip: Ipv4Addr,
...
/// Perform ARP discovery with cancellation support (Linux only).
///
/// Checks the cancel flag periodically during sending and receiving.
/// Returns early with `Err` containing "cancelled" message if cancel flag is set.
#[cfg(target_os = "linux")]
pub async fn discover_hosts_arp_cancellable(
    interface: &str,
    network: Ipv4Net,
    rate_limit_pps: Option<u32>,
    timeout: Duration,

...
impl WpaCracker {
    ...
    pub fn crack_wordlist(&mut self, wordlist_path: &Path) -> Result<CrackResult> {
        if !wordlist_path.exists() {
            return Err(WirelessError::System(
                format!("Wordlist not found: {:?}", wordlist_path)
            ));
        }
        ...
        for i in 0..=99_999_999u32 {
            ...
            if attempts % self.config.progress_interval == 0 {
                ...
                log::info!(
                    ...
                    attempts, rate, remaining, password
                );
            }
        }
    ...
    pub fn crack_common_patterns(&mut self) -> Result<CrackResult> {
        ...
        log::info!("Starting common patterns attack ({} patterns)", patterns.len());
...
pub fn generate_common_passwords() -> Vec<String> {
    ...
    let common = [
        ...
        "qwerty12", "qwertyui", "qwerty123", "password1",
        "password123", "iloveyou", "sunshine", "princess",
        "admin123", "welcome1", "monkey12", "dragon12",
        "master12", "letmein1", "trustno1", "baseball",
        "football", "superman", "batman12", "whatever",
    ];

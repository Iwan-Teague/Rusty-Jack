...
impl InjectionSocket {
    ...
    pub fn new(interface: &str) -> Result<Self> {
        ...
        if fd < 0 {
            return Err(WirelessError::Socket(
                format!("Failed to create raw socket: {}", io::Error::last_os_error())
            ));
        }
        ...
        if bind_result < 0 {
            ...
            return Err(WirelessError::Socket(
                format!("Failed to bind socket to interface: {}", io::Error::last_os_error())
            ));
        }
    ...
    pub fn send_raw(&self, data: &[u8]) -> Result<usize> {
        ...
        if sent < 0 {
            return Err(WirelessError::Injection(
                format!("Send failed: {}", io::Error::last_os_error())
            ));
        }
...
impl Injector {
    ...
    pub fn inject_deauth_burst(&mut self, bssid: MacAddress, client: Option<MacAddress>, reason: crate::frames::DeauthReason, count: u32) -> Result<InjectionStats> {
        let mut stats = InjectionStats::default();

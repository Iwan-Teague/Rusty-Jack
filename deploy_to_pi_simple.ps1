#!/usr/bin/env pwsh
# Simple Deploy Script for Rustyjack on Pi (cleaned)
...
Write-Host "  2. Run the install script on the Pi" -ForegroundColor Gray
Write-Host "  3. Drop into an interactive shell on the Pi" -ForegroundColor Gray
Write-Host "  4. Keep this PowerShell window open at the end" -ForegroundColor Gray
Write-Host ""
...
echo ""
echo "All commands finished successfully. Dropping to an interactive shell"
exec bash -l
'@
...
try {
    ...
    Write-Host "" -ForegroundColor Cyan
    Write-Host "Script complete. Press Enter to close this window." -ForegroundColor Cyan

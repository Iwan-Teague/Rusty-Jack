...
pub fn pkill_pattern(pattern: &str) -> Result<usize> {
    ...
    process::pkill(pattern)
        .map_err(|e| WirelessError::System(format!("Failed to kill processes matching '{}': {}", pattern, e)))
}

pub fn pkill_pattern_force(pattern: &str) -> Result<usize> {
    use rustyjack_netlink::process;
    process::pkill_force(pattern)
        .map_err(|e| WirelessError::System(format!("Failed to kill -9 processes matching '{}': {}", pattern, e)))
}
...
pub fn pgrep_pattern(pattern: &str) -> Result<Vec<i32>> {
    ...
    process::pgrep(pattern)
        .map_err(|e| WirelessError::System(format!("Failed to find processes matching '{}': {}", pattern, e)))
}
...
pub fn process_running(pattern: &str) -> Result<bool> {
    ...
    process::process_running(pattern)
        .map_err(|e| WirelessError::System(format!("Failed to check if process '{}' is running: {}", pattern, e)))
}

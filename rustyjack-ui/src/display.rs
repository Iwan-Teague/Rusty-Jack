...
impl Display {
    ...
    fn draw_attack_metrics(&mut self, status: &StatusOverlay) -> Result<()> {
        ...
        if status.mitm_victims > 0 && y <= 118 {
            Text::with_baseline(
                &format!("MITM Vic:{}", status.mitm_victims),
                Point::new(4, y),
                self.text_style_small,
                Baseline::Top,
            )
    ...
    fn draw_loot_summary(&mut self, status: &StatusOverlay) -> Result<()> {
        ...
        if y <= 119 {
            Text::with_baseline(
                &format!("Pkts:{}", status.packets_captured),
                Point::new(6, y),
                self.text_style_small,
                Baseline::Top,
            )
        ...
        if y <= 119 {
            Text::with_baseline(
                &format!("Time:{}h{}m", uptime_hrs, uptime_mins),
                Point::new(6, y),
                self.text_style_small,
                Baseline::Top,
            )
        ...
        if y <= 119 {
            if status.creds_found > 0 {
                Text::with_baseline(
                    &format!("• NTLM:{}", status.creds_found),
                    Point::new(6, y),
                    self.text_style_small,
                    Baseline::Top,
                )
...
impl Display {
    ...
    pub fn draw_dashboard(&mut self, view: DashboardView, status: &StatusOverlay) -> Result<()> {
        ...
        println!(
            "NET RX: {} MB  TX: {} MB",
            status.net_rx_bytes / 1_048_576,
            status.net_tx_bytes / 1_048_576
        );
        println!(
            "Rate: RX:{:.1} KB/s TX:{:.1} KB/s",
            status.net_rx_rate / 1024.0,
            status.net_tx_rate / 1024.0
        );
        println!(
            "Packets: {}  Creds: {}",
            status.packets_captured, status.creds_found
        );

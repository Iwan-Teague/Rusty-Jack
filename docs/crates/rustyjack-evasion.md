...
- `passive`: monitor-mode enable/disable with TX power adjustments; channel setting.
...
- Linux netlink for most operations; guarded fallbacks for host builds.
...
- `passive.rs`: monitor-mode enable/disable with TX power adjustments and channel setting. Tracks active monitor interfaces to clean up; guarded on non-Linux with explicit errors.
- `state.rs`: `StateManager` for combined interface state (MAC, monitor IFs, tx power). Includes monitor deletion via `rustyjack-netlink`, MAC restore via netlink, tx power restore, auto-restore on drop when enabled; Linux-gated implementations with non-Linux bailouts.
